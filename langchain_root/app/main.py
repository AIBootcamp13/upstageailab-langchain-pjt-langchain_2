...
class MainApp:
    ...
    # def run_ingest(self):
    #     print("[INGEST] 아직 구현 전입니다. 다음 단계에서 'crawler'와 'sql' 모듈을 붙일게요.")
    def run_ingest(self):
        """
        1) RSS에서 글 목록을 읽고
        2) 각 글의 본문을 추출한 뒤
    ...
    # 2) 색인: 청킹/임베딩 → Chroma 업서트
    def run_index(self):
        print("[INDEX ] 아직 구현 전입니다. 다음 단계에서 'vector_store'와 'llm(임베딩)'을 붙일게요.")
...
def main():
    ...
    app.run_ingest()
    app.run_index()
    app.run_qa("최근 생성형 AI 규제 동향을 요약해줘.")
